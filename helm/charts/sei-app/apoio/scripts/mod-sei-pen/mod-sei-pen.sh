...
if [ -z "$MODULO_PEN_UNIDADE_ASSOCIACAO_SEI" ] || [ -z "$MODULO_PEN_UNIDADE_ASSOCIACAO_PEN" ]; then
    echo "Nenhum valor informado para MODULO_PEN_UNIDADE_ASSOCIACAO_PEN ou MODULO_PEN_UNIDADE_ASSOCIACAO_SEI, nao sera feita a config automatica para esse parametro"
else
    echo "Configurando MODULO_PEN_UNIDADE_ASSOCIACAO_SEI e MODULO_PEN_UNIDADE_ASSOCIACAO_PEN "
    php -r "
        require_once '/opt/sei/web/SEI.php';
        \$conexao = BancoSEI::getInstance();
        \$conexao->abrirConexao();
        \$conexao->executarSql(\"insert ignore into md_pen_unidade(id_unidade, id_unidade_rh) values ($MODULO_PEN_UNIDADE_ASSOCIACAO_SEI, $MODULO_PEN_UNIDADE_ASSOCIACAO_PEN)\");
        "
