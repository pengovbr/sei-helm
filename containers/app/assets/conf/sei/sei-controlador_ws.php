...
try {
  ...
  switch (PaginaSEI::GET('servico')) {
    ...
    default:
      foreach ($SEI_MODULOS as $objModulo) {
        if (($strArq = $objModulo->executar('processarControladorWebServices', PaginaSEI::GET('servico'))) != null) {
          break;
        }
      }
  ...
  InfraPagina::montarHeaderDownload(basename($strArq));
